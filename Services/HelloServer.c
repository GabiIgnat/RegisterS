/* HelloServer.c - This is an example of a server that receives a string and sends "Hello <string>"
...
int main (int argc, char *argv[])
{
  ...
  while (1)
  {
      ...
      ERROR_CONTINUE( (td = (struct thData*)malloc(sizeof(struct thData))) == NULL,
                    "[server]Eroare la malloc.\n");	
      td -> idThread = i++;
      td -> cl = client;
      
      //thread CREATION
      ERROR_CONTINUE(pthread_create(&th[i], NULL, &treat, td) != 0,
                    "[server]Eroare la phtread_create().\n");		      
...
bool answer(void *arg)
{
  ...
  // {
    ...
    ERROR_RET(strcpy(msg_answer,"Hello ") == NULL, false,
              "[Thread %d] Eroare la strcpy().\n",tdL.idThread);
    ERROR_RET(strcat(msg_answer,msg_received) == NULL, false,
